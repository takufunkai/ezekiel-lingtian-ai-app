...
**No scenario outcome or score is claimed anywhere in this report**, because none has been produced: no fixture case has been reconciled against a live gateway from this repository. Every profile document committed to it is hand-authored — the schema example, the validator and renderer test fixtures, and the samples the results UI is seeded with. That is checkable rather than a promise, though not by one field. `src/engine.ts` stamps a `model` field onto every profile it emits, and three of the five committed profiles carry none at all — which is why the results UI renders them as "hand-written". The other two, `out.json` and `results/out.json` (byte-identical), do carry `"model": "minimax-m3"`, and are still not live output: their `generatedAt` is the round `2026-08-13T12:00:00Z` lifted from the worked example, their subject is the Nimbus format-example entity rather than any of the three fixture cases, and `minimax-m3` ceased to be the configured default when the model allowlist landed. A `model` field is an assertion whoever wrote the file chose to make, not evidence that a model wrote it. What *is* evidenced is the code around the model call, and that is evidenced by tests that run offline.
